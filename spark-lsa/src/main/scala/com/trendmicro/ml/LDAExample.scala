...
object LDAExample {
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setAppName("LDA").setMaster("local").set("spark.hadoop.validateOutputSpecs", "false")
    val sc = new SparkContext(conf)

    val data = MLUtils.loadLibSVMFile(sc, "D:\\03datafolder\\data\\input\\keywordidfs.csv")

    val dataset = data.map(x => (x.label.toLong,x.features))
