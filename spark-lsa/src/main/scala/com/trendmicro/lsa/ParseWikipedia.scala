...
import com.trendmicro.skyaid.common.XmlInputFormat
...
object ParseWikipedia {
  ...
  def documentTermMatrix(docs: RDD[(String, Seq[String])], stopWords: Set[String], numTerms: Int,
                         sc: SparkContext): (RDD[Vector], Map[Int, String], Map[Long, String], Map[String, Double], Array[(String, Int)]) = {
    ...
    docTermFreqs.cache()
    val tempdoccolect = docTermFreqs.collect();
    //println("tempdoccolect: " + tempdoccolect.array.toString)
    val docIds = docTermFreqs.map(_._1).zipWithUniqueId().map(_.swap).collectAsMap()
    //println("docIds: " + docIds.toString)
    saveDocIds(RunLSA.category2Id, docIds)

    val docFreqs = documentFrequenciesDistributed(docTermFreqs.map(_._2), numTerms)
    //println("Number of terms: " + docFreqs.size)
    saveDocFreqs(RunLSA.termsfreqs, docFreqs)

    val numDocs = docIds.size

    val idfs = inverseDocumentFrequencies(docFreqs, numDocs)

    saveMap2local(RunLSA.termsidfs, idfs)
    ...
    (vecs, termIds, docIds, idfs, docFreqs)
  }
  ...
  : Array[(String, Int)] = {
    ...
    //count times
//        val docFreqs = docTermFreqs.flatMap(_.iterator).reduceByKey(_+_)
  ...
  def saveMap2local(path: String, saveDocIds: Map[String, Double]) {
    val ps = new PrintStream(new FileOutputStream(path))
    for ((doc, freq) <- saveDocIds) {
      ps.println(doc + "," + freq)
    }
    ps.close()
  }

  def saveDocIds(path: String, saveDocIds: Map[Long, String]) {
    val ps = new PrintStream(new FileOutputStream(path))
    for ((doc, freq) <- saveDocIds) {
      ps.println(doc+","+freq)
    }
  ...
  def saveUserItemScore(path: String, uis: Seq[(String,String,Double)]) {
    val ps = new PrintStream(new FileOutputStream(path))
    for ((u,i, s) <- uis) {
      ps.println(u+","+i+","+s)
    }
  ...
  def saveResult(path: String, saveDocIds: Seq[(String, Int, Seq[(String, Double)])]) {
    val ps = new PrintStream(new FileOutputStream(path))
    for ((doc, freq, mseq) <- saveDocIds) {
      ps.println(doc + "," + freq + "," + seq2String(mseq))
    }
    ps.close()
  }

  def seq2String(seqr: Seq[(String, Double)]): String = {
    var first = true
    var tem: String = ""
    for ((doc, freq) <- seqr) {
      if (first) {
        tem = tem.concat(doc + "," + freq)
        first = false
      } else {
        tem = tem.concat(",").concat(doc + "," + freq)
      }
    }
    //    println("seq2String: " + tem)
    tem
  }
