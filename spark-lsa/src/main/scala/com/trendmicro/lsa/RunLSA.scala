...
object RunLSA {
  ...
  val dataFilterLength: Int = 7
  val topResult: Int = 3

  def run(pic: Int) = {
    inputOutputPath("D:\\03datafolder\\data\\input\\0711\\gcclsa\\lsadata_" + pic + "_1.csv",
      "D:\\03datafolder\\data\\input\\0711\\gcclsa\\lsadata_" + pic + "_2.csv",
      "D:\\03datafolder\\data\\output\\0711\\lsatestresult_" + pic + ".csv",
      "D:\\03datafolder\\data\\output\\0711\\allcategory_" + pic + ".csv",
      "D:\\03datafolder\\data\\output\\0711\\testCategory_" + pic + ".csv",
      "D:\\03datafolder\\data\\output\\0711\\allcategoryIds_" + pic + ".csv",
      "D:\\03datafolder\\data\\output\\0711\\alltermsfreqs_" + pic + ".csv",
      "D:\\03datafolder\\data\\output\\0711\\alltermsidfs_" + pic + ".csv")

//            inputOutputPath("D:\\03datafolder\\data\\input\\0711\\result-callin-complete.csv",
//              "D:\\03datafolder\\data\\input\\0711\\callintest.csv",
//              "D:\\03datafolder\\data\\output\\0711\\lsatestresult.csv",
//              "D:\\03datafolder\\data\\output\\0711\\allcategory.csv",
//              "D:\\03datafolder\\data\\output\\0711\\testcallin74Category.csv",
//              "D:\\03datafolder\\data\\output\\0711\\allcategoryIds.csv",
//              "D:\\03datafolder\\data\\output\\0711\\alltermsfreqs.csv",
//              "D:\\03datafolder\\data\\output\\0711\\alltermsidfs.csv")
...
//          muserItemScore = muserItemScore.++(printRelevantDocs(x.trim))
//        }
//        mTestFile.foreach(cat => muserItemScore = muserItemScore.++(printRelevantDocs(cat.trim)))
//        saveUserItemScore("D:\\03datafolder\\data\\output\\0711\\userItemScore.csv", muserItemScore)
    //        printRelevantDocs("15DUSD")
    //        printRelevantDocs("1G3000")
   ...
   */
    ...
    val jpSeqrdd = filteredPlainText.map(sa => (sa(1), GccJpTextSegmentation.wordSegmentationForJPWords(sa(6)).toArray(new Array[String](0)).toSeq))

    saveCategorycount(testcategoryCount, jpSeqrdd)
    val maprdd = jpSeqrdd.map(filterArrayArrayRdd(docFreqsArray, _)).filter(_._2.size > 2)
  ...
  def main(args: Array[String]) {
    ...
    val clusterArray = Array(15, 30, 50, 100, 150, 0)
    for (i <- clusterArray) {
      run(i)
    }
//        run(0)
  ...
  : (RDD[Vector], Map[Int, String], Map[Long, String], Map[String, Double], Array[(String, Int)]) = {
    ...
    val plainTextSeq = filteredPlainText.map(sa => (sa(1), GccJpTextSegmentation.wordSegmentationForJPWords(sa(6)).toArray(new Array[String](0)).toSeq))
    ...
    val filterRDD = plainTextSeq.filter(_._2.size > 1)
